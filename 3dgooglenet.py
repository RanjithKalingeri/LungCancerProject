...
class Inception_block(nn.Module):
    def __init__(
        ...
        self.branch2 = nn.Sequential(
            conv_block(in_channels, red_3x3, kernel_size=(1, 1, 1)),
            conv_block(red_3x3, out_3x3, kernel_size=(3, 3, 3), padding=(1, 1, 1)),
        )

        self.branch3 = nn.Sequential(
            conv_block(in_channels, red_5x5, kernel_size=(1, 1, 1)),
            conv_block(red_5x5, out_5x5, kernel_size=(5, 5, 5), padding=(2, 2, 2)),
        )

        self.branch4 = nn.Sequential(
            nn.MaxPool3d(kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
            conv_block(in_channels, out_1x1pool, kernel_size=(1, 1, 1)),
        )
...
if __name__ == "__main__":
    # N = 3 (Mini batch size)
    x = torch.randn(1, 1, 224, 224, 250)
    model = GoogLeNet(aux_logits=False, num_classes=2)
    print(model(x))
